团队协作充能平台
创建了一个具有视觉冲击力的互动网页，适合在游戏、年会、活动中使用，通过替换产品图片（png），可以在现实生活每进行一个环节得出一个分数后（总分100）输入到输入框，团队所有人一起努力累计进度100%，将成功点亮设备图片！以及提前在代码中预留各种可自行更改设置并且注释，方便直接替换不同品牌活动、背景图片、产品图片，以及调整位置这个网页设计为全屏显示，适合在大型屏幕上展示，营造出震撼的视觉效果和团队协作氛围。
设计思路：这个网页模拟了一个能量充能系统，通过团队协作输入进度值，逐步点亮设备图片。主要特点包括：
...
这个网页非常适合在团队活动、年会或产品发布会上使用，通过以下步骤操作：

团队协作：团队成员轮流输入1-100之间的数值

充能过程：每次输入后按回车键，系统会播放充能特效

进度累积：进度会累积到设备图片上，逐步点亮

完成庆祝：当进度达到100%时，会播放庆祝特效和音效
